    ...
    for (antenna_list.items) |antenna_1| {
        for (antenna_list.items) |antenna_2| {
            if (antenna_1.type == antenna_2.type and !std.meta.eql(antenna_1, antenna_2)) {
                var antinode_pos = Position{ .i = antenna_2.i, .j = antenna_2.j };
                while (antinode_pos.is_not_out(length)) {
                    if (!antinode_pos.get_val(bool, antinodes, length)) {
                        antinode_count += 1;
                        antinode_pos.set_val(bool, true, &antinodes, length);
                    }
                    antinode_pos.i += (antenna_2.i - antenna_1.i);
                    antinode_pos.j += (antenna_2.j - antenna_1.j);
                }
            }
        }
    }
