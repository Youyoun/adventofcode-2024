...
fn run(input: [:0]const u8) i64 {
    ...
    var map = std.AutoHashMap(i64, i64).init(a);
    ...
    while (it.next()) |first_number_str| {
        ...
        const gop = map.getOrPut(second_number) catch unreachable;
    ...
    for (list1.items) |right| {
        const similarity_score = right * (map.get(right) orelse 0);
...
test "example" {
    ...
    try std.testing.expectEqual(@as(i64, 31), result);
}
