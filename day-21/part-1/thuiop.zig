...
fn get_shortest_recur(new_pos: Position, prev_pos: Position, rec_num: usize, keyboard_type: KeyboardType) usize {
    if (rec_num == 0) {
        return 1;
    }
    ...
    if (!unsafe_xfirst) {
        ...
        for (new_seq.items) |char| {
            ...
            new_length += get_shortest_recur(new_pos_next, prev_pos_next, rec_num - 1, .Directional);
...
fn find_sequence_length(seq: []const u8) usize {
    ...
    for (seq) |char| {
        ...
        total_length += get_shortest_recur(new_pos, prev_pos, 3, .Numerical);
        prev_pos = new_pos;
    }
    return total_length;
}
