...
const Position = struct {
    ...
    inline fn second_cond(current_val: u8, current_pos: Position, dir: Direction, field_grid: Grid(u8)) bool {
        const other_is_border = switch (dir) {
            .West => current_pos.j == 0,
            .East => current_pos.j == 0,
            .North => current_pos.i == 0,
            .South => current_pos.i == 0,
        };
        var ok: bool = other_is_border;
        if (!ok) {
            const prev_pos = switch (dir) {
                .West => current_pos.next(.North),
                .East => current_pos.next(.North),
                .North => current_pos.next(.West),
                .South => current_pos.next(.West),
            };
            const prev_val = field_grid.get(prev_pos);
            ok = !(prev_val == current_val and first_cond(current_val, prev_pos, dir, field_grid));
        }
        return ok;
    }
    ...
    fn get_perimeter(current_pos: Position, field_grid: Grid(u8)) i64 {
        ...
        inline for ([4]Direction{ .North, .South, .West, .East }) |dir| {
            if (first_cond(current_val, current_pos, dir, field_grid)) {
                if (second_cond(current_val, current_pos, dir, field_grid)) {
                    perimeter += 1;
                }
            }
        }
