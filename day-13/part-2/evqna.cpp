...
std::vector<Machine> parse_input(const std::string& str) {
  auto parse_machine = [](const std::string& s) {
    ...
    return Machine{{ax, ay}, {bx, by}, {px + OFFSET, py + OFFSET}};
  };
...
int64_t token_cost(const Machine& m) {
  ...
  int u1 = a1 / c, v1 = b1 / c;
  int64_t w1 = m.prize.x / c;
  Vec2 bezout1 = extended_gcd(a1, b1);
  int64_t x1 = w1*bezout1.x, y1 = w1*bezout1.y;

  int a2 = m.bA.y, b2 = m.bB.y;
  int u2 = a2 / d, v2 = b2 / d;
  int64_t w2 = m.prize.y / d;
  Vec2 bezout2 = extended_gcd(a2, b2);
  int64_t x2 = w2*bezout2.x, y2 = w2*bezout2.y;
  ...
  int64_t k1 = (v2*(y2-y1) - u2*(x1-x2)) / det;
  int64_t x = x1 - k1*v1;
  int64_t y = y1 + k1*u1;
