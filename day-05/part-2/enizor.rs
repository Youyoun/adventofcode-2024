...
fn fix_order<const N: usize>(rules: &bitset::ArrayBitSet<N>, updates: &mut [usize]) -> usize {
    let mut sorted = true;
    ...
    if sorted {
        ...
        updates.sort_unstable_by(|lhs, rhs| {
            if rules.test(lhs * 100 + rhs) {
                Ordering::Less
            } else {
                Ordering::Greater
            }
...
fn run(input: &str) -> usize {
    ...
    while i < input.len() {
        ...
        match b {
            b'\n' => {
                res += fix_order(&rules, &mut update);
                update.clear();
                update.push(0)
            }
            b',' => update.push(0),
