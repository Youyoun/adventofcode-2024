...
class ThChSubmission(SubmissionPy):
    def run(self, s: str):
        ...
        """
        initial_wires, cables = s.split("\n\n")
        wires = {}
        for line in initial_wires.split("\n"):
        ...
        for cable in cables.split("\n"):
            ...
            queue.append((a, operator, b, output))

        while queue:
            a, operator, b, output = queue.popleft()
        ...
        return int(
            "".join(
                str(value)
                for wire, value in sorted(
                    [
                        (wire, value)
                        for wire, value in wires.items()
                        if wire.startswith("z")
                    ],
                    key=lambda wire: wire[0],
                    reverse=True,
                )
            ),
            2,
